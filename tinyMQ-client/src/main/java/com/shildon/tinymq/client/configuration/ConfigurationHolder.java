...
public final class ConfigurationHolder {
    ...
    public List<Pair<String, Integer>> getServerInfos() {
        return Arrays.stream(this.configuration.getServers().split(","))
                .map(it -> {
                    String[] serverInfo = it.split(":");
                    return new Pair<>(serverInfo[0], Integer.valueOf(serverInfo[1]));
                })
    ...
    public int maxChannelSize() {
        return this.configuration.getMaxChannelSize();
    }
